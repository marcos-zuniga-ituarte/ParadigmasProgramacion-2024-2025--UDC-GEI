...
(* Exponenciación entera optimizada *)
...
;;
(* Función de hash mejorada para los estados de las torres *)
  ...
  let rec aux l =
    ...
    | h :: t ->
        ...
        if is_stable nuevo_estado && nuevo_estado <> (a, b, c) then
          ...
          aux t
  in
  (aux movimientos)(*[(List.hd (aux movimientos))]*)  (* Llama a la auxiliar con la lista de movimientos predeterminada *)
;;
...
let rec filtrar_local secuencias=
    let rec aux sec visitados=
      match sec with 
          [] -> []
          | (a,b)::t -> let hash=hash_state b in if existe (hash) visitados then aux t visitados else (a,b)::(aux t (visitados @ [hash]))
    in aux secuencias []
;;
...
let iterar secuencias=
    ...
    in filtrar_local (aux secuencias)
;;
...
(* Funciones auxiliares para acceder a los elementos de una tupla de tres *)
let fst3 (x, _, _) = x;;
let snd3 (_, y, _) = y;;
let trd3 (_, _, z) = z;;

(* Función de filtrado con las listas invertidas para comparar desde la cima *)
let rev_filtrar_por_puntaje_final estado_final secuencias =
  (* Calcular los puntajes de cada secuencia en relación al estado final con listas invertidas *)
  let puntajes = List.map (fun (a, b) -> 
    (a, b, puntaje_estado (List.rev (fst3 b), List.rev (snd3 b), List.rev (trd3 b))
                            (List.rev (fst3 estado_final), List.rev (snd3 estado_final), List.rev (trd3 estado_final)))) secuencias in
  (* Obtener el puntaje máximo de la lista *)
  let max_puntaje = List.fold_left (fun acc (_, _, p) -> max acc p) 0 puntajes in
  (* Filtrar solo las secuencias que tengan el puntaje máximo *)
  List.fold_right (fun (a, b, p) acc -> if p = max_puntaje then (a, b) :: acc else acc) puntajes []
...
let buscar_solucion2 sec rev_sec=
    ...
    if l=[] then []
    else let (a,b) = List.hd l in (buscar_solucion b sec) @ (List.rev a)
;;
...
(* Función auxiliar para encontrar la torre que no contiene el disco más grande *)
let torre_auxiliar torre_inicial torre_destino =
  match (torre_inicial, torre_destino) with
  | (1, 2) | (2, 1) -> 3
  | (1, 3) | (3, 1) -> 2
  | (2, 3) | (3, 2) -> 1
  | _ -> failwith "Torre inválida"
;;
...
let estado_intermedio estado_inicial estado_final =
  ...
  let disco_mayor =
    match (t1_ini, t2_ini, t3_ini) with
    | (x :: _, _, _) | (_, x :: _, _) | (_, _, x :: _) -> x
    | _ -> failwith "Estado inicial vacío"
  in
  let torre_inicial =
    if List.mem disco_mayor t1_ini then 1
    else if List.mem disco_mayor t2_ini then 2
    else 3
  in
  let torre_destino =
    if List.mem disco_mayor t1_fin then 1
    else if List.mem disco_mayor t2_fin then 2
    else 3
  in
...
let hanoi n (a1, b1, c1) (a2, b2, c2) =
  ...
  then raise (Invalid_argument "Estados no estables") 
    ...
    let estado_intermedio_calculado = estado_intermedio estado_inicial estado_final in
    let secuencias = [([], estado_inicial)] in
    let rev_secuencias = [([], estado_final)] in
    let visitados_inicial = [] in
    let rev_visitados_inicial = [] in
    let rec aux1 secuencias2 visitados2 rev_secuencias2 rev_visitados2=
        ...
        let sec_filtered=filtrar_por_puntaje_final estado_intermedio_calculado sec_filtered in
        let rev_sec_filtered=filtrar_por_puntaje_final estado_intermedio_calculado rev_sec_filtered in
        ...
        let rev_visitados3=actualizar_visitados rev_sec_filtered rev_visitados2 in
            if encontrado<>[] 
            then encontrado
            else if sec_filtered = [] || rev_sec_filtered = [] then [] else aux1 (sec_filtered) visitados3 (rev_sec_filtered) rev_visitados3
