...
let rec existe_sec posibles visitados =
  match posibles with
      [] -> []
    | (a,b)::t -> if existe (hash_state b) visitados then [(a,b)] else filtrar t visitados 
;;
...
let hanoi n (a1, b1, c1) (a2, b2, c2) =
  ...
  then raise (Invalid_argument "Estados no estables") 
    ...
    let secuencias=iterar secuencias in
    let rev_secuencias = [([], estado_final)] in
    let rev_secuencias=iterar rev_secuencias in
    let visitados_inicial = actualizar_visitados secuencias [] in
    let rev_visitados_inicial = actualizar_visitados rev_secuencias [] in
