...
let breadth_aux f a =
  let rec aux cola2 = function
      [] -> if cola2=[] then None else aux [] (List.rev cola2)
    | GT(x,[])::t -> if f x then Some x else aux cola2 t
    | GT(x,ln) :: t -> if f x then Some x else aux ((List.rev ln) @ cola2) (t)  (* ineficiente *)
  in aux [] [a]
;;

let breadth_find f b = let solucion = breadth_aux f b in if solucion = None then raise Not_found else Option.get(solucion);;

let breadth_find_opt f b = breadth_aux f b;;
...
*)
let rec from_stbin b =
  if StBinTree.is_leaf b then GT(StBinTree.root b, []) else
  let l,r = (StBinTree.left_b b), StBinTree.right_b b in
  GT(StBinTree.root b, (if StBinTree.is_leaf l then [GT(StBinTree.root l,[])] else [from_stbin l])@(if StBinTree.is_leaf r then [GT(StBinTree.root r,[])] else [from_stbin r]))
;;