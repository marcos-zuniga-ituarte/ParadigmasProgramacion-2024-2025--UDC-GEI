...
let rec cut_below f b1 = if f (root b1) then Leaf(root b1) else SBT(cut_below f (left_b b1), root b1, cut_below f (right_b b1))
;;
...
let rec from_bin b = 
    ...
    else if BinTree.is_empty (BinTree.left_b b) || BinTree.is_empty (BinTree.right_b b) then raise (Failure "from_bin") else SBT(from_bin(BinTree.left_b b),BinTree.root b,from_bin(BinTree.right_b b))
;;
